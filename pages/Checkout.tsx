...
export const Checkout: React.FC = () => {
      ...
      try {
        if (!('Square' in window)) {
          ...
          if (!existingScript) {
            ...
            script.onerror = () => setSquareError('Failed to load Square');
  ...
  const handlePayment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const success = await placeOrder(result.token, totalAfterDiscount, formData);
      if (success) {
        navigate('/success');
      }
    } catch {
  ...
  const isFormComplete =
    ...
    !!formData.city &&
    !!formData.zipCode;
  const discountAmount = appliedCoupon ? pricing.totalPrice * appliedCoupon.percent : 0;
  const totalAfterDiscount = Math.max(0, pricing.totalPrice - discountAmount);
