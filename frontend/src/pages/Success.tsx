...
export const Success: React.FC = () => {
  const { orders, deliveryDetails, clearCart } = useApp();
  const location = useLocation();
  const navigate = useNavigate();
  const stateOrder = (location.state as { order?: any } | undefined)?.order;
  const lastOrder = stateOrder || orders[0];
  const totalText =
    lastOrder?.totalPrice !== undefined && lastOrder?.totalPrice !== null
      ? `$${Number(lastOrder.totalPrice).toFixed(2)}`
      : '--';
            ...
            {lastOrder?.items?.length ? (
              <div className="space-y-4">
                {lastOrder.items.map(item => (
                  <div
                    key={`${item.meal?._id || item.meal?.id || item.meal?.name}-${item.baseOption || 'default'}`}
              ...
              <div className="text-sm text-gray-500">
                Your order is confirmed. Visit your dashboard for details.
              </div>
            )}
          </div>
